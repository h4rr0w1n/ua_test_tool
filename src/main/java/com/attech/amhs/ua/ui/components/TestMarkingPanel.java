...
/**
 * UI Panel for displaying all sent and received messages connected to the
 * User/O/R Address.
...
public class TestMarkingPanel extends JPanel {
    ...
    private String formatMessage(MessageLog message) {
        ...
        // Detailed X.400 Payload (which contains Sender, Recipient, Subject, Priority, AMHS fields)
        if (message.getX400Payload() != null && !message.getX400Payload().isEmpty()) {
            sb.append(message.getX400Payload());
        } else {
            // Fallback if payload isn't set
